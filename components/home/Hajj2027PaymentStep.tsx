...
export default function Hajj2027PaymentStep({ enquiryId, customerName, onReset }: PaymentStepProps) {
  ...
  const submitPayment = async () => {
    ...
    setError('');
    try {
      const res = await fetch('/api/hajj2027/payment', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ enquiryId, method: selected, reference }),
      });
  ...
  if (stage === 'success') {
    return (
      <div className="bg-gradient-to-br from-green-50 to-emerald-50 border border-green-200 rounded-3xl p-10 sm:p-12 text-center shadow-sm">
        ...
        <div className="mt-6 p-4 rounded-2xl bg-white border border-green-100 max-w-md mx-auto text-sm text-gray-600">
          ...
          <p className="mt-1">
            ...
            <span className="font-semibold text-gray-900">
              ৳{HAJJ_2027_BOOKING_FEE.toLocaleString('en-IN')}
            </span>
          </p>
        </div>
        <button
  ...
  return (
    <div className="bg-white rounded-3xl border border-stone-200 shadow-sm p-6 sm:p-10">
      ...
      <div className="flex items-start justify-between gap-4 mb-6">
        <div>
          <p className="text-xs font-semibold text-[#2d6a4f] tracking-widest uppercase">Step 3 · Booking Fee</p>
          <h3 className="mt-1 text-2xl font-bold text-gray-900">প্যাকেজ বুকিং পেমেন্ট</h3>
          <p className="mt-1 text-sm text-gray-600">
            Hajj 2027 বুকিং নিশ্চিত করতে অগ্রিম ৳ {HAJJ_2027_BOOKING_FEE.toLocaleString('en-IN')} জমা দিন
          </p>
        </div>
        <div className="hidden sm:block text-right">
          <div className="inline-flex flex-col items-end px-4 py-2.5 rounded-2xl bg-gradient-to-br from-[#2d6a4f] to-[#1b4332] text-white shadow-sm">
            <span className="text-[10px] font-semibold uppercase tracking-wider opacity-80">Booking fee</span>
      ...
      {(stage === 'detail' || stage === 'submitting') && selected && (
        <div>
          ...
          {selected === 'bank' && (
            <BankDetail reference={reference} onReferenceChange={setReference} />
          )}
          {selected === 'card' && <CardDetail />}
          {selected === 'cash' && <CashDetail />}
  ...
  return (
    <div className="space-y-5">
      <div className="rounded-2xl bg-gradient-to-br from-stone-50 to-white border border-stone-200 p-5">
        ...
        <p className="mt-1 text-sm text-gray-600">
          Amount: <span className="font-bold text-gray-900">৳ 30,000</span>
        </p>
      </div>
  ...
  return (
    <div className="space-y-5">
      <div className="rounded-2xl bg-gradient-to-br from-blue-50/60 to-white border border-blue-100 p-5 space-y-1.5 text-sm">
        ...
        <Row label="Amount" value="৳ 30,000" bold />
      </div>
